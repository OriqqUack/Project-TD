...
using UnityEngine;
//퀘스트를 관리 quest 등록 및 보고, 저장과 같은 전반적인 역할 담당
...
public class QuestSystem : MonoBehaviour
{
    ...
    public Quest Register(Quest quest)
    {
        ...
        {
            newQuest.onCompleted += OnQuestSCompleted;
    ...
    {
        foreach (var quest in quests.ToArray()) // 사본을 만드는 이유는 퀘스트 시스템이 돌아가는중에 퀘스트가 완성될수도 있어서 / 목록이 바뀌면 에러가 나기 때문
    ...
    private JArray CreateSaveDatas(IReadOnlyList<Quest> quests)
    {
        ...
        {
            if(quest.IsSavable)
                saveDatas.Add(JObject.FromObject(quest.ToSaveData())); //SaveData를 Json형태로 변환시킨 뒤 JsonArray에 넣어줌
        }
        return saveDatas;
    }

    //JToken 자리에 CreateSaveData가 들어올거임
    ...
    #region Callback
    private void OnQuestSCompleted(Quest quest)
    {
    ...
    {
        activeAchievements.Remove(quest);
